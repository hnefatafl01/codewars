...
function toObj(d) { 
    let townsArr = d.split('\n');
    const formatted = townsArr.map((val) => {
        var town = {
            name: val.substring(0, val.indexOf(':')),
            months: {}
        };
        var monthsArr = val.slice(val.indexOf(':') + 1, val.indexOf('\n')).split(',')
        monthsArr.forEach( (m) => {
            let mod = m.slice(0, m.indexOf(' '))
            town.months[mod] = Number(m.slice(m.indexOf(' ')));
        })
        return town
    });
